...
def gen_random_batch(batch_size, cfg):
    ...
    batch["attention_mask"] = torch.ones(
        size=(batch_size, cfg.max_seq_len), dtype=torch.int64
    )
    return batch
...
def main(cfg):
    ...
    reproducibility.seed_all(cfg.get("seed", 17))
    ...
    if load_path is None:
        print("Error: checkpoint load_path is required for exporting.")
        sys.exit(-1)
    ...
    trainer = Trainer(model=orig_model, load_path=load_path, load_weights_only=True)
    # load export model with torch attention
    attn_impl = cfg.model.get("attn_impl", "torch")
    if attn_impl == "triton" or attn_impl == "flash":
        print(f"Replacing {cfg.model.attn_impl} attention with torch causal attention")
        cfg.model.attn_impl = "torch"
        export_model = build_composer_model(cfg.model)
        trainer = Trainer(
            model=export_model, load_path=load_path, load_weights_only=True
        )
        ...
        for idx in range(cfg.model.n_layers):
            ...
            torch_causal_attn.mhsa.in_proj_weight = orig_model.model.transformer.blocks[
                idx
            ].causal_attn.mhsa.Wqkv.weight
            torch_causal_attn.mhsa.in_proj_bias = orig_model.model.transformer.blocks[
                idx
            ].causal_attn.mhsa.Wqkv.bias
            torch_causal_attn.mhsa.out_proj.weight = (
                orig_model.model.transformer.blocks[
                    idx
                ].causal_attn.mhsa.out_proj.weight
            )
            torch_causal_attn.mhsa.out_proj.bias = orig_model.model.transformer.blocks[
                idx
            ].causal_attn.mhsa.out_proj.bias
            export_model.model.transformer.blocks[idx].causal_attn = torch_causal_attn
    else:
    ...
    with tempdir_ctx as tempdir:
        ...
        if is_remote_store:
            ...
            save_object_store.upload_object(remote_path.lstrip("/"), local_save_path)
